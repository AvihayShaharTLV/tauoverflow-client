    ...
    const renderDiscussions = () => {
        return discussions.map((discussion, index) => {
            return <li key={index} style={{ 'fontWeight': selectedDiscussion?.id === discussion.id ? 'bold' : 'normal' }}
                className= "cursor-pointer break-words underline truncate px-2 py-1 rounded-xl hover:bg-gray-200 dark:hover:bg-gray-700" onClick={() => setSelectedDiscussion([discussion, index+1])}>{index+1}. {discussion.title}</li>
        })
    }
